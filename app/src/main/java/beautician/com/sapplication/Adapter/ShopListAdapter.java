...
public class ShopListAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        String picUrl= "http://applicationworld.net/beautician/profile/";

         if(!_pos.getPhoto1().isEmpty()) {
            Picasso.with(_context).load(picUrl+_pos.getPhoto1()).into(holder.pic1);
        }
        if(!_pos.getPhoto2().isEmpty()) {
            Picasso.with(_context).load(picUrl+_pos.getPhoto2()).into(holder.pic2);
        }
        if(!_pos.getPhoto3().isEmpty()) {
            Picasso.with(_context).load(picUrl+_pos.getPhoto3()).into(holder.pic3);
        }
